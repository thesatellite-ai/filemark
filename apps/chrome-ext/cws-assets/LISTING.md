...
### 4.3 `host_permissions: file:///*` + `optional_host_permissions: *://*/` — 957/1000 ✓

```text
Filemark declares only "file:///*" as required host permission. "*://*/" is OPTIONAL — not granted at install. The user must toggle "Enable on all sites" in Filemark's options, at which point Chrome shows its own permission prompt. A fresh install cannot read any web page. The required "file:///*" lets declarativeNetRequest redirect local .csv / .tsv URLs (which Chrome would otherwise download) into Filemark's viewer. The optional "*://*/", when granted, registers additional DNR rules that redirect remote URLs ending in supported extensions (.md, .mdx, .json, .jsonc, .csv, .tsv, .sql, .prisma, .dbml) to the viewer — so e.g. https://raw.githubusercontent.com/.../README.md renders as proper markdown. Filemark does NOT read, scrape, log, modify, or transmit any web page content. It does NOT inject scripts into browsed pages. MV3 strict CSP applies (no unsafe-eval); every renderer is bundled at build time. Verifiable in DevTools' Network tab.
```
